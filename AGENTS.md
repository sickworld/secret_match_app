...
- After every push, verify that the GitHub Actions run for the pushed commit SHA was created and report its link and current status. Do not block delivery while queued or running jobs finish. If the run has already failed or been cancelled during the task, investigate and fix it where possible; otherwise leave the final result visible in GitHub.
