...
Ein nach `main` gemergter Pull Request mit dem Titel `release` oder `Release: <Version>` löst nach dem vollständig grünen Push-Run den getrennten Workflow `App Store Connect Release` aus. Er archiviert und signiert Teilnehmer- und Admin-App nacheinander und lädt beide Builds nach App Store Connect. Direkte Pushes, offene PRs und normale PR-Titel können keinen Upload auslösen. Benötigte Environment Secrets und die einmalige Apple-Einrichtung stehen unter [Automatischer App-Store-Connect-Release](APP-STORE-CONNECT-RELEASE.md).
