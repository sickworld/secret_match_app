...
Unter **System & Archiv** schließt der gemeinsame Assistent das aktuelle Event unter einem frei wählbaren Namen ab. Der Server archiviert Daten, Logs und vollständige Statistik, bevor er den Live-Stand auf null setzt. In **Statistik** kann anschließend zwischen dem aktuellen Event und allen archivierten Events gewählt werden; historische Auswertungen lassen sich als PDF oder CSV teilen sowie umbenennen, wiederherstellen oder nach exakter Sicherheitsbestätigung löschen. Vor einer Wiederherstellung legt der Server den aktuellen Live-Stand transaktional als benanntes Sicherheitsarchiv ab. Nummern, PINs, Schnelltexte, Medien und globale Einstellungen werden dabei nicht überschrieben; Teilnehmer- und Billboard-Sitzungen werden beendet und eine neue Event-ID trennt alte Offline-Queues ab. Die Archiv-Tabelle im WordPress-Modul ist die einzige kanonische Quelle; beide App-Targets verwenden dieselben Models und `APIService`-Aufrufe.
...
Diese Funktionen benötigen das WordPress-Modul ab Version `2026.09.10.5`; das Modul muss vor oder zusammen mit diesem App-Build veröffentlicht werden.
