...
Admin-Eingabefelder verwenden gemeinsam `AdminKeyboardTextField` beziehungsweise `AdminKeyboardTextEditor`. Im integrierten iPad-Adminbereich öffnen sie die große appinterne Zahlen- oder QWERTZ-Tastatur; mehrzeilige Schnelltexte unterstützen dabei eine eigene Neue-Zeile-Taste. Der eigenständige iPhone-Admin-Target rendert über dieselben Komponenten weiterhin native iOS-Eingabefelder. Der Admin-Passwort-Login bleibt aus Sicherheits- und Zeichenkompatibilitätsgründen auf beiden Targets eine native Secure-Field-Eingabe.
