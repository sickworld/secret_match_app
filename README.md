...
Der Workflow `iOS Quality` läuft bei Pushes und Pull Requests auf `main`, wöchentlich sowie manuell. Er zeigt getrennte Jobs für Code-Qualität, Teilnehmer-Build, Admin-Build, Tests und Coverage sowie CodeQL. Der Quality-Job prüft zuerst die Repository-Hygiene, synchrone App-Versionen und SwiftFormat; danach starten die übrigen Prüfungen parallel. Die Tests laufen auf einem verfügbaren iPad-Simulator und erzwingen mindestens 80 % Core Coverage. Diese stabile Pflichtmetrik umfasst produktiven Swift-Code außerhalb der SwiftUI-Verzeichnisse `Views` und `Components`; die gesamte App-Coverage inklusive compiler-generierter SwiftUI-Zeilen wird zusätzlich informativ ausgewiesen. Die ausgelassenen Oberflächen bleiben durch ihre eigenen netzwerkfreien Render-Smoke-Tests geprüft.
...
Ein nach `main` gemergter Pull Request aus einem Branch mit dem Präfix `release/` schaltet nach den vollständig grünen Pflichtjobs im selben `iOS Quality`-Push-Run automatisch die Release-Jobs frei; der PR-Titel ist dafür nicht relevant. Sie archivieren und signieren Teilnehmer- und Admin-App nacheinander und laden beide Builds nach App Store Connect. Im offenen PR sind diese Jobs erwartungsgemäß übersprungen, weil ausschließlich der getestete Merge-Commit auf `main` hochgeladen werden darf. Direkte Pushes und Pull Requests aus anderen Branches können keinen Upload auslösen. Benötigte Environment Secrets und die einmalige Apple-Einrichtung stehen unter [Automatischer App-Store-Connect-Release](APP-STORE-CONNECT-RELEASE.md).
...
Der biometrische Admin-Schnellzugriff prüft einen gespeicherten Sitzungstoken vor dem Öffnen des Adminbereichs beim Server. Ist der kurzlebige Token abgelaufen, erneuert die App ihn nach erfolgreichem Face ID beziehungsweise Touch ID mit der gerätegebunden im iOS-Schlüsselbund gespeicherten Admin-Anmeldung. Ein geändertes Passwort, ein nicht erreichbarer Server und eine tatsächlich abgelaufene Anmeldung erhalten getrennte, verständliche Rückmeldungen; ein ausdrückliches Admin-Logout entfernt Token und biometrische Anmeldung vollständig vom Gerät.

Unter **System & Archiv → Admin-Zugänge** verwalten iPhone- und iPad-Adminbereich dieselben benannten Admin-Zugänge. Jeder Zugang besitzt ein eigenes, ausschließlich serverseitig gehasht gespeichertes Passwort und kann einzeln widerrufen werden; dabei werden seine zuordenbaren aktiven Sitzungen sofort beendet. Das bisherige Standardpasswort bleibt für bestehende Installationen kompatibel. Die gemeinsame Implementierung liegt in `AdminDashboardView`, `AdminCredential` und `APIService`, nicht in getrennten Target-spezifischen Admin-Flows.

Unter **System & Archiv** schließt der gemeinsame Assistent das aktuelle Event unter einem frei wählbaren Namen ab. Der Server archiviert Daten, Logs und vollständige Statistik, bevor er den Live-Stand auf null setzt. In **Statistik** kann anschließend zwischen dem aktuellen Event und allen archivierten Events gewählt werden; historische Auswertungen lassen sich als PDF oder CSV teilen sowie umbenennen, wiederherstellen oder nach exakter Sicherheitsbestätigung löschen. Die gemeinsame Event-Bilanz zeigt Teilnahmequote, Aktivität pro beteiligter Nummer, offene und erfolgreiche Wünsche je Match-Typ, Match-Dauer, Freitext- und Rückzugsquote, Feedback-Mittelwerte, Eventdauer, Peak-Viertelstunde und protokollierte Betriebsereignisse. Der Server berechnet diese Werte aus dem vorhandenen Snapshot auch für ältere Archive rückwirkend; die App speichert dafür keine zusätzlichen personenbezogenen Daten. Vor einer Wiederherstellung legt der Server den aktuellen Live-Stand transaktional als benanntes Sicherheitsarchiv ab. Nummern, Schnelltexte, Medien und globale Einstellungen werden dabei nicht überschrieben. Teilnehmer-PINs werden dagegen bei Abschluss und Wiederherstellung vollständig zurückgesetzt und nicht archiviert, sodass beim nächsten Login die vorhandene PIN-Selbstvergabe startet. Teilnehmer- und Billboard-Sitzungen werden beendet und eine neue Event-ID trennt alte Offline-Queues ab. Die Archiv-Tabelle im WordPress-Modul ist die einzige kanonische Quelle; beide App-Targets verwenden dieselben Models und `APIService`-Aufrufe.

Unter **Eventsteuerung → Bildschirmschoner & Sponsoren** verwalten iPhone- und iPad-Adminbereich denselben WordPress-Medienkatalog. Bilder können aus Fotos oder Dateien hochgeladen, aktiviert beziehungsweise pausiert sowie getrennt für Bildschirmschoner und Sponsorleiste freigegeben werden; Begleittext, Anzeigedauer, Reihenfolge und ein optionaler heller Logohintergrund bleiben editierbar. Auch die Leerlaufzeit bis zum Start des Login-Bildschirmschoners ist dort zentral zwischen 15 Sekunden und 10 Minuten pflegbar. Der helle Hintergrund wird in sämtlichen Bildschirmschoner-, Sponsorleisten- und Admin-Vorschauen gleich dargestellt. Ab Modulversion `2026.09.09.8` werden die vier bisher fest eingebauten Motive und Begleittexte einmalig in die WordPress-Mediathek und diesen Katalog übernommen; beim FICKEN-Standardmotiv ist der helle Hintergrund voraktiviert. Die Teilnehmer-App prüft Einstellungen und Katalog im Vordergrund mindestens einmal pro Minute, speichert beides lokal und aktiviert einen neuen Medienstand erst, wenn sämtliche zugehörigen Bilder vollständig geladen und dekodiert wurden. Ohne Netz bleiben deshalb Leerlaufzeit und letzter vollständiger Katalog erhalten; auf einer frischen Installation gelten 60 Sekunden und die eingebauten Match&Play- und Partnerbilder als Fallback.

Das Billboard verwendet ab Modulversion `2026.09.09.9` eine einzige stabile Live-Ansicht ohne Seitenrotation. Matches, Job-Aktionspuls und die Aktivität der letzten zehn Minuten bleiben gleichzeitig sichtbar; ein neues Match wird für die in der Admin-Steuerung gewählte Dauer groß eingeblendet. Der Top-16-Modus zeigt alle 16 Platzierungen gleichzeitig im 4×4-Raster.

Auf dem iPhone öffnet die Kopfleiste das kompakte Admin-Menü. Das mobile **Dashboard** konzentriert sich auf Live-Status, Geräte und Kennzahlen; die einzelnen Werkzeuge erscheinen dort nicht zusätzlich, sondern bleiben als direkte Punkte im Admin-Menü erreichbar. Auf dem iPad nutzt der Adminbereich die volle Breite ohne permanente Sidebar; alle Funktionen liegen in der adaptiven Übersicht **Aktionen**. Sie enthält Livefeed, Aktionen-, Request- und Match-CRUD, Teilnehmerverwaltung, Eventsteuerung, Event-Check, Sendungsdiagnose, Protokoll, Statistik, Feedback sowie System und Reset. Nummernsuche und Admin-Logout bleiben direkt in der Kopfzeile erreichbar, Detailseiten erhalten einen Rückweg zu **Aktionen**. Der Aktionseditor öffnet auf dem iPad direkt in einer erhöhten, gut erreichbaren Sheet-Position und lässt sich bei Bedarf vollständig aufziehen. Die Inhalte nutzen adaptive Grids, statt das iPhone-Layout lediglich zu verbreitern.

Admin-Eingabefelder verwenden gemeinsam `AdminKeyboardTextField` beziehungsweise `AdminKeyboardTextEditor`. Im integrierten iPad-Adminbereich öffnen sie die große appinterne Zahlen- oder QWERTZ-Tastatur als einheitliches Overlay über der abgedunkelten aktuellen Ansicht; mehrzeilige Schnelltexte unterstützen dabei eine eigene Neue-Zeile-Taste. Die QWERTZ-Tastatur orientiert sich an einer normalen Bildschirmtastatur und wechselt über eigene Tasten zwischen Buchstaben, Zahlen, Sonderzeichen und einer kuratierten Emoji-Auswahl. Innerhalb der Emoji-Ebene wechselt eine eigene Taste zwischen allgemeinen und frecheren Event-Emojis, ohne die Tastaturhöhe zu verändern. Auch der iPad-Admin-Login verwendet diese gemeinsame Tastatur. Der eigenständige iPhone-Admin-Target rendert über dieselben Komponenten weiterhin native iOS-Eingabefelder.
...
Das empfohlene Router-, Access-Point- und lokale Serversystem ist im [Match&Play Event-Netzwerk](EVENT-NETZWERK-SETUP.md) dokumentiert. Das Dokument trennt den bereits nutzbaren stabilen WLAN-Aufbau vom noch zu implementierenden vollständigen lokalen Eventbetrieb.

Die Teilnehmeranmeldung bleibt aus Sicherheitsgründen vollständig servergebunden: Ohne erfolgreiche Prüfung von Nummer und PIN gibt es keinen Offline-Login. Ein bereits erkannter Verbindungsabbruch beendet Anmeldung, PIN-Anlage und Gender-Speicherung sofort; bei scheinbar vorhandenem, aber unbrauchbarem Netz erzwingt die App für diese interaktiven Authentifizierungsanfragen einen harten Gesamtabbruch nach drei Sekunden mit einer verständlichen Meldung. Eingaben bleiben für einen erneuten Versuch erhalten.
...
Die Teilnehmernavigation bündelt Matches, offene Interessen und erhaltene Aktionen unter **Deine Übersicht**. Der iPad-Sidebarpunkt heißt dafür bewusst kurz **Aktionen**; innerhalb der Übersicht unterscheiden die Reiter eindeutig zwischen **Erhaltene Aktionen** und **Von dir gesendet**. Drei klar erklärte Reiter unterscheiden gegenseitig bestätigte Matches, noch offene Interessen und direkt empfangene Aktionen. Einheitliche Karten stellen die Eventnummer zuerst dar; Typ- und Nummernfilter funktionieren in allen drei Bereichen. Nach einer einmaligen Registrierung während eines Teilnehmer-Logins übermittelt das iPad im Vordergrund unabhängig vom aktuellen Login alle 30 Sekunden Akkustand, Ladezustand, App-Version und die gesamte lokale Queue. Der dafür ausgestellte Geräte-Token liegt nur im iOS-Schlüsselbund. Die Admin-App zeigt Geräte an, deren letzter Heartbeat höchstens drei Minuten zurückliegt, und kennzeichnet iPads auf der Loginseite als „Keine Nummer angemeldet“.

In jeder Teilnehmerübersicht steht **Alle** auf dem iPad als erster Filter an derselben Position. In der Aktionsübersicht folgen direkt daneben die kompakten Richtungsreiter **Erhalten** und **Gesendet** sowie anschließend die Typfilter in einer gemeinsamen horizontalen Zeile. Alle Auswahlpunkte verwenden dieselbe kontrastreiche Chip-Darstellung mit identischer Höhe, Typografie, Kontur und Auswahlrückmeldung. Richtung und Typ bleiben fachlich unabhängige Filterdimensionen.
...
Beim Eventabschluss werden die anonymen Feedbacks zusammen mit den übrigen Eventdaten im benannten Archiv gesichert und anschließend aus dem aktiven System gelöscht.
