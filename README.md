...
Ein nach `main` gemergter Pull Request mit dem Titel `release`, `Release <Version>` oder `Release: <Version>` schaltet nach den vollständig grünen Pflichtjobs im selben `iOS Quality`-Push-Run die Release-Jobs frei. Sie archivieren und signieren Teilnehmer- und Admin-App nacheinander und laden beide Builds nach App Store Connect. Im offenen PR sind diese Jobs erwartungsgemäß übersprungen, weil ausschließlich der getestete Merge-Commit auf `main` hochgeladen werden darf. Direkte Pushes und normale PR-Titel können keinen Upload auslösen. Benötigte Environment Secrets und die einmalige Apple-Einrichtung stehen unter [Automatischer App-Store-Connect-Release](APP-STORE-CONNECT-RELEASE.md).
