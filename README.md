...
Admin-Eingabefelder verwenden gemeinsam `AdminKeyboardTextField` beziehungsweise `AdminKeyboardTextEditor`. Im integrierten iPad-Adminbereich öffnen sie die große appinterne Zahlen- oder QWERTZ-Tastatur; mehrzeilige Schnelltexte unterstützen dabei eine eigene Neue-Zeile-Taste. Auch der iPad-Admin-Login verwendet diese Tastatur und ergänzt dafür zwei Reihen mit den üblichen Passwort-Sonderzeichen. Der eigenständige iPhone-Admin-Target rendert über dieselben Komponenten weiterhin native iOS-Eingabefelder.
