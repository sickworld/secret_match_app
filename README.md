...
Ein nach `main` gemergter Pull Request mit dem Titel `release`, `Release <Version>` oder `Release: <Version>` löst nach dem vollständig grünen Push-Run den getrennten Workflow `App Store Connect Release` aus. Die Versionsnummer muss dreiteilig numerisch sein, zum Beispiel `Release 2026.09.14`. Der Workflow archiviert und signiert Teilnehmer- und Admin-App nacheinander und lädt beide Builds nach App Store Connect. Direkte Pushes, offene PRs und normale PR-Titel können keinen Upload auslösen. Benötigte Environment Secrets und die einmalige Apple-Einrichtung stehen unter [Automatischer App-Store-Connect-Release](APP-STORE-CONNECT-RELEASE.md).
